...
import { IconLists } from "@/atoms/icons";
...
function IconTable() {
  ...
  const SlillsArray: (keyof IconItem)[] = ["FrontEnds", "BackEnds", "Etc"];

  return (
    <div className={styles.icon}>
      {SlillsArray.map((skillArray: string, index: number) => (
        <div key={skillArray} className={styles.category}>
          ...
          <div className={styles.grid}>
            {IconList[index]?.[skillArray as keyof IconItem]?.map(
              (icon: { name: string; icon: React.ReactNode }) => (
                <span key={icon.name} className={styles.iconItem}>
